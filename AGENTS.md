...
Follow the branch, commit, and pull request naming rules in `docs/development.md`. In particular:

1. Use a Conventional Commit type as the branch prefix, such as `feat/`, `fix/`, or `docs/`. Never use a tool- or vendor-specific prefix such as `codex/`.
2. Format commit subjects as Conventional Commits and append `(#<pr-number>)` once a pull request exists.
3. Use the same Conventional Commit subject for the pull request title, without the pull request number.
