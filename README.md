...
AI coding agents default to reading whole files. With pitlane-mcp, they fetch only the symbol they need — **532× less tokens** on a Rust codebase ([ripgrep](https://github.com/BurntSushi/ripgrep)), **418×** on C++ ([LevelDB](https://github.com/google/leveldb)), **133×** on C ([Redis](https://github.com/redis/redis)), **125×** on Go ([Gin](https://github.com/gin-gonic/gin)), **112×** on Java ([Guava](https://github.com/google/guava)), **65×** on C# ([Newtonsoft.Json](https://github.com/JamesNK/Newtonsoft.Json)), **61×** on Ruby ([RuboCop](https://github.com/rubocop/rubocop)), **53×** on TypeScript ([Hono](https://github.com/honojs/hono)), **20×** on Python ([FastAPI](https://github.com/fastapi/fastapi)), and Bash ([bats-core](https://github.com/bats-core/bats-core))¹.
...
| Metric | ripgrep | FastAPI | Hono | Redis | LevelDB | Gin | Guava | Newtonsoft.Json | bats-core | RuboCop |
|---|---|---|---|---|---|---|---|---|---|---|
| Indexing time (min / median, 5 runs) | 26 ms / 28 ms | 32 ms / 34 ms | 17 ms / 18 ms | 103 ms / 104 ms | 10 ms / 12 ms | 11 ms / 11 ms | 239 ms / 246 ms | 84 ms / 86 ms | 2 ms / 2 ms | 50 ms / 52 ms |
| Peak RAM (first-run) | 40.1 MB | 37.8 MB | 31.3 MB | 94.1 MB | 24.1 MB | 21.9 MB | 201.9 MB | 76.7 MB | 10.3 MB | 47.3 MB |
| Index size on disk | 1.1 MB | 1.6 MB | 275 KB | 3.9 MB | 398 KB | 354 KB | 28.4 MB | 3.1 MB | 52.5 KB | 3.1 MB |
| Token efficiency — median | **532×** | **20×** | **53×** | **133×** | **418×** | **125×** | **112×** | **65×** | N/A¹ | **61×** |
...
| `search_symbols` latency | 144 µs | 30 µs | 36 µs | 673 µs | 61 µs | 47 µs | 65 µs | 395 µs | 7.9 µs | 428 µs |
| `get_symbol` latency | 3.4 µs | 3.8 µs | 3.8 µs | 10.8 µs | 2.6 µs | 3.5 µs | 6.8 µs | 5.8 µs | 4.2 µs | 5.0 µs |
| `get_file_outline` latency | 96 µs | 52 µs | 5.0 µs | 1.06 ms | 72 µs | 59 µs | 33 µs | 2.7 µs | 16.0 µs | 44.5 µs |
| `get_project_outline` latency | 362 µs | 1.93 ms | 273 µs | 2.24 ms | 227 µs | 150 µs | 24.5 ms | 1.84 ms | 43.3 µs | 1.88 ms |
| `find_usages` latency | 19.7 ms | 29.6 ms | 11.7 ms | 29.7 ms | 2.0 ms | 139 µs | 3.3 ms | 964 µs | 485 µs | 1.60 ms |
